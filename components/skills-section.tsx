...
export function SkillsSection({ initialSkills }: SkillsSectionProps) {
  ...
  return (
    <section id="skills" className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-12 sm:py-16 md:py-20 lg:py-24">
      {/* Section Header */}
      <div className="flex items-center gap-3 mb-6 sm:mb-8">
        <span className="font-mono text-emerald-600 dark:text-emerald-400 text-base sm:text-lg font-bold">02.</span>
        ...
        <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-zinc-200/70 dark:bg-zinc-900/90 border-b border-black/10 dark:border-white/10 font-mono text-xs text-zinc-500 dark:text-zinc-400">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="ml-2 sm:ml-3 flex items-center gap-1.5 text-zinc-600 dark:text-zinc-400">
              ...
              <span className="text-[11px] sm:text-xs">neofetch --skills</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <div
                    key={cat}
                    className="p-3.5 sm:p-5 rounded-lg border border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-black/30 space-y-3"
                  >
                    ...
                    <div className="flex flex-wrap gap-2 sm:gap-2.5 pt-1">
                      {categorySkills.map((skill) => (
                        <div
                          ...
                          className="group relative overflow-hidden px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[11px] sm:text-xs font-mono border border-black/10 dark:border-white/10 bg-white/60 dark:bg-zinc-900/80 text-zinc-800 dark:text-zinc-200 hover:border-emerald-500/50 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all shadow-sm"
                        >
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {typeof skill.proficiency === "number" && skill.proficiency > 0 && (
                              <span className="text-[9px] sm:text-[10px] text-zinc-400 dark:text-zinc-500 opacity-60 group-hover:opacity-100 shrink-0">
                                {skill.proficiency}%
                              </span>
                            )}
                          </div>
