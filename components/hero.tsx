  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 pt-6 sm:pt-10 md:pt-16 pb-12 sm:pb-16 md:pb-20">
      ...
      <div className="w-full max-w-4xl mx-auto rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-[#111116]/80 shadow-2xl backdrop-blur-md overflow-hidden transition-all">
        ...
        <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 sm:py-3 bg-zinc-200/60 dark:bg-zinc-900/80 border-b border-black/10 dark:border-white/10 font-mono text-xs text-zinc-500 dark:text-zinc-400">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="ml-2 sm:ml-3 hidden sm:flex items-center gap-1.5 text-zinc-600 dark:text-zinc-400">
              <Terminal className="w-3.5 h-3.5 text-emerald-500" />
              <span>bash - 80x24</span>
            </div>
          </div>
          <div className="text-zinc-600 dark:text-zinc-400 font-mono tracking-tight text-[10px] sm:text-xs truncate max-w-[180px] sm:max-w-none">
            {siteContent.terminalPromptUser}
          </div>
        </div>

        {/* Terminal Window Content */}
        <div className="p-4 sm:p-6 md:p-8 font-mono text-xs sm:text-sm space-y-5 sm:space-y-6 text-zinc-800 dark:text-zinc-200 leading-relaxed">
          {/* $ whoami command */}
          <div>
            ...
            <div className="mt-2 pl-3 sm:pl-4 border-l-2 border-emerald-500/40 dark:border-emerald-400/30">
              <h1 className="text-lg sm:text-2xl font-bold text-zinc-900 dark:text-white tracking-tight">
                {displayName}
              </h1>
              <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400 mt-1 font-sans leading-normal">
                {displayIdentity}
              </p>
            </div>
          </div>
          ...
          {/* Status Line */}
          <div className="pt-1">
            <span className="inline-flex items-center px-2.5 sm:px-3 py-1 rounded-md text-[11px] sm:text-xs font-mono font-medium bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 dark:border-emerald-500/30">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse mr-2 inline-block shrink-0" />
              <span className="truncate">{displayStatus}</span>
            </span>
          </div>

          {/* Closing Prompt with Blinking Cursor */}
          <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400 pt-1">
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">$</span>
      ...
      {/* CTA Buttons */}
      <div className="w-full max-w-4xl mx-auto mt-6 sm:mt-8 flex flex-col sm:flex-row items-stretch sm:items-center justify-start gap-3 sm:gap-4">
        <a
          href={siteContent.cta.exploreProjects.href}
          className="inline-flex items-center justify-center gap-2 min-h-[44px] px-6 py-3 rounded-lg font-mono text-xs sm:text-sm font-semibold bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/20 transition-all duration-150 active:scale-[0.98]"
        >
        ...
        <a
          ...
          className="inline-flex items-center justify-center gap-2 min-h-[44px] px-6 py-3 rounded-lg font-mono text-xs sm:text-sm font-semibold border border-black/15 dark:border-white/15 bg-white/40 dark:bg-white/5 hover:bg-black/5 dark:hover:bg-white/10 text-zinc-800 dark:text-zinc-200 transition-all duration-150 active:scale-[0.98]"
        >
      ...
      <div className="w-full max-w-4xl mx-auto mt-10 sm:mt-12 pt-6 sm:pt-8 border-t border-black/10 dark:border-white/10">
        <h2 className="font-mono text-[11px] sm:text-xs font-semibold tracking-wider text-zinc-500 dark:text-zinc-400 uppercase mb-3 sm:mb-4">
          CORE TECHNOLOGIES
        </h2>
